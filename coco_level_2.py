...
def main():
    ...
    for test_fold in TEST_FOLDS:
        ...
        for split_path in cv_splits:
            ...
            out_dir = f"experiments/LTDETR_coco_level_2/{slugify(run_name)}"

            lightly_train.train_object_detection(
                out=out_dir,
                model=str(Path(__file__).parent / "hybrid_checkpoints" / "hybrid_coco_head_dinov3_backbone.pt"),
                overwrite=True,
                batch_size=4,
                steps=steps,
                accelerator="gpu",
